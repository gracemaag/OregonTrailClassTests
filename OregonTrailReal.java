 ...
 * This product includes naming the characters (aside from Anne Marie King, the wagon leader), 
 * buying vital items from the game's store, a medicine item to increase wagon health and strategy,
 ...
 * and images of different places along the trail. This code also includes various random events relating to 
 * the rivers, weather, health of wagon members, health of oxen, and trail status.  
 * This product also includes a sewing minigame, which requires clicking precision from the user to 
 * 'sew clothes', which are then sold for money that can be used to purchase more goods.
 */
...
import oregonTrail.WagonMember;
...
public class OregonTrailReal {
	...
	private Timer clock;
	private JButton sewButton = new JButton("Sewing");
	private JButton stitchButton = new JButton();
	private MiniGameMechanics mechanics;
	...
	private Item medicine = new Item(5,20);
	...
	private JButton medicineButton = new JButton("Use Medicine");
	...
	// create radio button group and its components
	...
	//intitalize some image components
	private ImageIcon trailImage = new ImageIcon ("C:\\Users\\rease\\Downloads\\trailImage1.png");
	private ImageIcon mapImage = new ImageIcon("C:\\Users\\rease\\Downloads\\map1.jpg");
	Image img = trailImage.getImage();
	Image newImg = img.getScaledInstance( 794, 472, Image.SCALE_SMOOTH);
	
	// create values for the state of the current game
	public static final boolean PAUSED = true;
	public static final boolean RESUMED = false;
	private boolean gameState = PAUSED; 
	
	// intitalize values to be store within the whole game
	...
	public static void main(String[] args)
	{
		EventQueue.invokeLater(new Runnable()
		{
			public void run()
			{
				try
				{
					OregonTrailReal window = new OregonTrailReal();
		...
		clock = new javax.swing.Timer(2500, new ActionListener()
		{
			public void actionPerformed(ActionEvent evt) 
			{
				...
				if(game.getDistanceToNext() == 0 && game.getTotalTraveled() != 0 ) 
				{
					...
					if(landmark.getLandmarkName().contains("Will")) 
					{
						controlBtn.setEnabled(false);
						clock.stop();
						JOptionPane.showMessageDialog(null,"You've made it to Willamette Valley! Thank you for playing.");
						outputTextArea.setText("You've made it to Willamette Valley! Thank you for playing.");
					}
				...
				else if(restCount > 0 || oxenChance == 1) 
				{
					...
					restCount--;
					// check to see if you have any oxen 
					if (oxenChance == 1)
					{
				...
				else 
				{
					...
					/**
					 * If Oxen method injuryChance returns 2, an oxen has died. If 3, an oxen is sick. Only one
					 * oxen can be sick at a time, if another gets sick, the currently sick one dies.
					 */
					...
					// Only runs through if there is at least one living member.
					if (livingMems != 0)
					{
						int randEvent = WagonMember.randomHealthEvent(); // Checks for a random event (Lost or Rough Trail)
						if (randEvent == 2) // # Corresponding to 'Lost Trail'
						{
							JOptionPane.showMessageDialog(null, "Lost Trail.");
						}
						else if (randEvent == 3) // # Corresponding to 'Rough Trail'
						{
							JOptionPane.showMessageDialog(null, "Rough Trail.");
						}
						
						int sickMember = WagonMember.illnessProb(members, livingMems); // Checks if a member is sick
						if (sickMember != -1 && sickMember != -2) // -1 and -2 indicate that no member is sick
						{
							String sickName = members.get(sickMember).memberName; // Gets name of person who will be sick
							if ((members.get(sickMember)).hasIllness == true) // If member already has an illness, they die
							{
							...
							{
								// Gets the name of the illness for the member, and flags them as sick
					...
					else
					{
						...
						frame.dispose(); // Game over if all members dead, frame closes
						System.exit(0);
					}
					...
					if (WagonMember.memberHealth >= 140)
					{
						...
						frame.dispose(); // Game over if health conditions go about 140, frame closes
						System.exit(0);
					}
					
					int ifBetter = WagonMember.isBetter(members,  livingMems); // Checks if any sick members get better
					if (ifBetter != -1) 
					{
						String betterName = members.get(ifBetter).memberName; // Gets the name of the better member 
				...
				{
					JOptionPane.showMessageDialog(null, outputWeather);
	...
	private void initialize()
	{
		...
		itemContents.add(medicine); // 5 - Medicine
		itemContents.add(water); // 6 - Water (Not changeable)
		...
		bareBonesBtn.setBackground(new Color(152, 147, 129));
		...
		meagerBtn.setBackground(new Color(152, 147, 129));
		...
		fillingBtn.setBackground(new Color(152, 147, 129));
		...
		currentSpeedSlider.setBackground(new Color(152, 147, 129));
		...
		lookAtSupplies.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				clock.stop();
				System.out.println(members.get(0).memberName + members.get(1).memberName + members.get(2).memberName + members.get(3).memberName) ;
				...
				" | Medicine: " + itemAmounts.get(5) + " | Water: " + itemAmounts.get(6));
			}
		});
		...
		choiceTF.addActionListener(new ActionListener() 
		{
			public void actionPerformed(ActionEvent e) 
			{
				...
				/* 
				 * gets out of action event if the user inputed an invalid number
				 * this will require the user to re-enter a value.
				 */
		...
		JLabel lblNewLabel = new JLabel("<html>1 - Wagon Wheel($10) <br> 2 - Wagon Axle($10) <br> 3 - Clothing($10) <br> 4 - Food($0.20/lb)  <br>5 - Oxen($20) <br> 6 - Medicine($20)</html>");
		...
		/**
		 * Allows the user to buy items from the general store by getting the 
		 * kind of item they want and the amount they want.
		 */
		...
		amountField.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (itemPricing * itemAmount <= totalMoney)
				{
					...
					totalWagonWeight += tempItem.getWeight() * itemAmount; // Adds the weight of the bought item to the wagon

					totalMoney -= (itemPricing * itemAmount); // Subtracts from their total money amount
		...
		JLabel lblYouShouldBe = new JLabel("<html> - You should be getting at least 200 pounds of food per member! If you don't want to starve. <br> <br> - At least two pairs of clothes for each person for those cold days! <br><br> -Also, I suggest at least 5 oxen... there's been a lot of sickness going around! <br> <br> - Make sure you get some medicine. Sickness is running rampant on the trail. Use your medicine items to boost your health!");
		lblYouShouldBe.setForeground(new Color(255, 255, 255));
		lblYouShouldBe.setFont(new Font("Perpetua", Font.BOLD, 14));
		lblYouShouldBe.setBounds(368, 109, 341, 275);
		...
		/**
		 * Checks if the user has any medicine to use, tells them if it has been used
		 * or not. Using a medicine item decreases the current health value by 20 (The lower the
		 * health value, the better.)
		 */
		medicineButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				int medicineValue = WagonMember.useMedicine(itemAmounts);
				if (medicineValue == 0)
				{
					JOptionPane.showMessageDialog(null, "Medicine Used!");
				}
				else if (medicineValue == -1)
				{
					JOptionPane.showMessageDialog(null, "No medicine left. Please buy more!");
				}
			}
		});
		medicineButton.setForeground(Color.WHITE);
		medicineButton.setFont(new Font("Tahoma", Font.PLAIN, 10));
		medicineButton.setBackground(Color.BLACK);
		medicineButton.setBounds(138, 731, 122, 23);
		frame.getContentPane().add(medicineButton);
		
		JLabel lblNewLabel_1 = new JLabel("");
		lblNewLabel_1.setOpaque(true);
		lblNewLabel_1.setBackground(new Color(152, 147, 129));
		lblNewLabel_1.setBounds(355, 487, 400, 262);
		frame.getContentPane().add(lblNewLabel_1);
		
		/**
		 * Opens sewing minigame window at anytime 
		 * 
		 * 
		 */
		sewButton.setFont(new Font("Tahoma", Font.PLAIN, 10));
		sewButton.setForeground(new Color(255, 255, 255));
		sewButton.setBackground(new Color(0, 0, 0));
		sewButton.setEnabled(false);
		sewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) 
			{
				sewingFrame.setVisible(true);
				frame.setVisible(false);
				clock.stop();
				JOptionPane.showMessageDialog(null, "Try your best to click the button as it moves around the screen.\n"
						+ "The more you click it, the more clothes you can make! Your grand total will be traded in for money.");
				mechanics = new MiniGameMechanics(stitchButton);
				mechanics.moveStitchButtonRandomly(); // Start moving the button
				
				/**
				 * Checks to see the number of clicks within the frame, also checks
				 * if the click is on the button or somewhere else in the frame.  
				 */
				sewingFrame.addMouseListener(new MouseAdapter() {
			        ...
			        public void mouseClicked(MouseEvent e) {
			            ...
			            {
			                mechanics.moveStitchButtonRandomly(); // moves button if misclicked
			            }
			            if(mechanics.getGameStatus()) 
			            {
			            	sewingFrame.setVisible(false);
							frame.setVisible(true);
							totalMoney += mechanics.notifyPlayer();
							clock.start();
			            }
			        }
			    });
		...
		sewButton.setBounds(10, 731, 122, 23);
		frame.getContentPane().add(sewButton);
